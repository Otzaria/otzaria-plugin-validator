...
```gitignore
# הערה — שורות ריקות ושורות שמתחילות ב‑# מתעלמים מהן
*.map            # glob לפי basename בכל עומק
src/             # תיקייה שלמה (וכל מה שתחתיה)
data/raw.json    # נתיב מעוגן לשורש התוסף
build/**         # ** חוצה מפרידי נתיב
!src/keep.js     # ! מחזיר קובץ שהוחרג ע"י כלל קודם
```
