...
const { getApiSpec, mergeWithFallback, DEFAULT_SPEC_URL } = require('./apiSpec')
...
async function main() {
  ...
  const legacyApiUrl = readInput('api-reference-url', '').trim()
  const apiUrl = readInput('spec-url', '').trim() || DEFAULT_SPEC_URL

  ga.startGroup('מאחזר מפרט SDK רשמי מ-GitHub')
  if (legacyApiUrl) {
    ga.warning(
      "הקלט 'api-reference-url' אינו בשימוש יותר — המפרט נקרא מ-spec.json " +
      "ולא מ-API_REFERENCE.md. השתמש ב-'spec-url'."
    )
  }
  const rawSpec = await getApiSpec(apiUrl)
  const spec = mergeWithFallback(rawSpec)
  if (rawSpec.source === 'remote') {
    ga.info(`✓ מפרט ה-SDK נטען בזמן אמת מ-${apiUrl}`)
    ga.info(`  הרשאות: ${spec.permissions.size}, methods: ${spec.apiMethods.size}, events: ${spec.events.size}`)
  } else {
    // אינו אזהרה: העותק המצורף הוא אותו ארטיפקט שמתפרסם, ולכן התוצאה זהה.
    ga.info(
      `מפרט ה-SDK לא נטען מ-GitHub (${rawSpec.error || 'שגיאה לא ידועה'}); ` +
      'נעשה שימוש בעותק המצורף (src/spec.json) — אותו ארטיפקט, ולכן אותה ' +
      'תוצאה. ייתכן שחסרים בו APIים שנוספו לאחר גרסת הוולידטור.'
    )
  }
  ga.endGroup()
...
function buildPluginFor(source, report, cache) {
  ...
  const built = buildOtzplugin(source.root, path.resolve(source.root, '..', outputName), manifest)
  ...
  if (built.metadataExcluded.length) {
    ga.info(
      `${built.metadataExcluded.length} קבצי מטא-דאטה הוחרגו: ${built.metadataExcluded.join(', ')}` +
        ' (החזרה לארכיון: שורת "!<נתיב>" ב-.otzignore)',
    )
  }
